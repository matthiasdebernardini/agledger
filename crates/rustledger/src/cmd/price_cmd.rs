...
pub fn run(args: &PriceArgs, price_config: &PriceConfig) -> Result<()> {
    ...
    run_with_writer(args, price_config, &mut handle)
}

/// Run the price command with price output written to `writer`.
pub fn run_with_writer<W: Write>(
    ...
    if args.list_sources {
        return list_sources(&registry, writer);
    }
    ...
    for symbol in &symbols_to_fetch {
        ...
        for (effective_currency, per_spec_mapping) in per_quote_jobs {
                ...
                && let Some(cached) = c.get(&key)
            {
                ...
                write_price(writer, symbol, &cached, args.beancount)?;
                continue;
            }
            ...
            match result {
                Ok(response) => {
                    ...
                    write_price(writer, symbol, &response, args.beancount)?;
                }
                Err(e) => {
                    if args.verbose {
            ...
            match source.fetch_price(&request) {
                Ok(response) => {
                    ...
                    if args.beancount {
                        ...
                        writeln!(writer, "{symbol}: {} {}", response.price, response.currency)?;
                    }
                }
                Err(e) => {
                    if args.verbose {
...
fn list_sources<W: Write>(registry: &PriceSourceRegistry, writer: &mut W) -> Result<()> {
    writeln!(writer, "Available price sources:")?;
    writeln!(writer)?;
    ...
    for name in sources {
        if let Some(source) = registry.get(name) {
            ...
            writeln!(writer, "  {name}{default_marker}{api_key_note}")?;

...
/// List available importers, writing output to `stdout`.
pub fn list_importers_with_writer<W: Write>(args: &Args, stdout: &mut W) -> Result<()> {
    // ===== TOML profiles =====
    //
    ...
    if let Some(config_path) = find_importers_config(args.config.as_deref())? {
        ...
        if config.importers.is_empty() {
            writeln!(stdout, "No TOML profiles in {}", config_path.display())?;
        } else {
            writeln!(stdout, "TOML profiles in {}:", config_path.display())?;
    ...
    writeln!(stdout, "Registered importer engines:")?;
...
/// Run the extract command with output written to `stdout`.
pub fn run_with_writer<W: Write>(args: &Args, file: &Path, stdout: &mut W) -> Result<()> {
    let registry = build_registry(args)?;

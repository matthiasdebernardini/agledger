//! agledger - agent-native CLI for rustledger.
//!
//! This binary is intentionally additive: `rledger` remains the upstream-style
//! clap CLI, while `agledger` exposes the same accounting commands through
//! agcli JSON envelopes for agents.
...
const SCHEMA_VERSION: &str = "agledger.v1";
...
fn build_cli() -> AgentCli {
    let cli = AgentCli::new("agledger", "Agent-native plain-text accounting CLI")
    ...
    debug_assert!(
        cli.audit().is_clean(),
        "agledger command tree should be agent-auditable"
    );
    cli
}
...
fn check_command(name: &'static str, description: &'static str) -> Command {
    Command::new(name, description)
        .usage(
            "agledger check [<file>] [--format <format>] [--json] [--verbose] [--quiet] \
             [--no-cache] [--auto] [--native-plugin <name>] [--lint <name>]",
        )
        .default_next_action(NextAction::new(
            "agledger check <file> --format=json",
...
fn query_command(name: &'static str, description: &'static str) -> Command {
    Command::new(name, description)
        .usage(
            "agledger query [<file>] <query...> [--file <file>] [--query-file <file>] \
             [--output <file>] [--format <format>] [--numberify] [--no-errors] [--verbose]",
        )
        .default_next_action(
            NextAction::new(
                "agledger query <file> \"SELECT account, sum(position) GROUP BY account\" --format=json",
            ...
            Box::pin(async move {
                ...
                if args.query.is_empty() && args.query_file.is_none() {
                    return Err(CommandError::new(
                        "query text is required for agledger query",
                        "MISSING_QUERY",
...
fn format_command(name: &'static str, description: &'static str) -> Command {
    Command::new(name, description)
        .usage(
            "agledger format [<file>...] [--output <file>] [--in-place] [--check] [--diff] \
             [--verbose]",
        )
        .default_next_action(NextAction::new(
            "agledger format <file> --check",
...
fn report_command(name: &'static str, description: &'static str) -> Command {
    Command::new(name, description)
        .usage(
            "agledger report [<file>] <report> [--file <file>] [--format <format>] [--verbose] \
             [--account <account>] [--limit <n>] [--period <period>] [--currency <currency>] \
             [--no-zero]",
        )
        .default_next_action(NextAction::new(
            "agledger report <file> balances --format=json",
...
fn doctor_command(name: &'static str, description: &'static str) -> Command {
    Command::new(name, description)
        .usage(
            "agledger doctor <subcommand> [args...] [--verbose] [--conversion <value>] \
             [--output <dir>] [--count <n>] [--seed <n>] [--skip-validation] [--manifest] \
             [--edge-cases-only]",
        )
        .default_next_action(NextAction::new(
            "agledger doctor <subcommand> [args...]",
...
fn price_command(name: &'static str, description: &'static str) -> Command {
    Command::new(name, description)
        .usage(
            "agledger price [<symbol>...] [--file <file>] [--currency <currency>] [--date <date>] \
             [--beancount] [--verbose] [--mapping <from:to>] [--source <source>] \
             [--source-cmd <cmd>] [--list-sources] [--no-cache] [--clear-cache] [--inactive] \
             [--undeclared] [--dry-run] [--clobber]",
        )
        .default_next_action(NextAction::new(
            "agledger price --file <file> --dry-run",
...
fn config_command(name: &'static str, description: &'static str) -> Command {
    Command::new(name, description)
        .usage("agledger config <show|path|edit|init|aliases> [--raw] [--format <format>] [--project] [--system] [--force]")
        .default_next_action(NextAction::new(
            "agledger config <subcommand> [--format=<format>]",
...
fn add_command(name: &'static str, description: &'static str) -> Command {
    Command::new(name, description)
        .usage(
            "agledger add [<file>] --quick <payee> <narration> <account> <amount> <account> \
             [--file <file>] [--date <date>] [--dry-run] [--yes] [--no-completion]",
        )
        .default_next_action(NextAction::new(
            "agledger add <file> --quick <payee> <narration> <account> <amount> <account> --dry-run",
...
fn compat_command() -> Command {
    ...
    .usage("agledger compat <install|uninstall> [--prefix <dir>]")
    .default_next_action(NextAction::new(
        "agledger compat <action> [--prefix=<dir>]",
...
fn lint_command() -> Command {
    Command::new("lint", "Run non-fatal advisory passes")
        .usage(
            "agledger lint transfers <file>... [--min-confidence <n>] [--date-window <days>] \
             [--amount-tolerance <amount>] [--apply] [--format <format>]",
        )
        .default_next_action(NextAction::new(
            "agledger lint <lint> <file> [--format=<format>]",
...
fn build_price_args(
    ...
    Ok(rustledger::cmd::price_cmd::PriceArgs {
        ...
        no_cache: bool_flag(req, "no-cache", None),
        ...
        dry_run: bool_flag(req, "dry-run", Some("n")),
...
fn build_add_args(
    ...
    if quick.as_ref().is_some_and(|items| items.len() < 4) {
        return Err(CommandError::new(
            ...
            "INVALID_QUICK_ARGS",
            "Use: agledger add <file> --quick <payee> <narration> <account> <amount> <account>",
        )
        .exit_code(agcli::ExitCode::USAGE));
    }
    Ok(rustledger::cmd::add_cmd::Args {
        ...
        date: string_flag(req, "date", Some("d")),
        dry_run: bool_flag(req, "dry-run", Some("n")),
        yes: bool_flag(req, "yes", Some("y")) || bool_flag(req, "no-input", None),
        quick: quick.take(),
...
fn profile_from_env_or_flag(req: &agcli::CommandRequest<'_>) -> Option<String> {
    req.flag("profile")
        .or_else(|| req.flag("P"))
        .map(str::to_string)
        .or_else(|| std::env::var("AGLEDGER_PROFILE").ok())
